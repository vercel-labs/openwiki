...
export async function POST(request: NextRequest) {
  ...
  const enforceGenerationRateLimitOnce = async () => {
    if (isConfiguredFeaturedRepository || generationRateLimitReserved) return;
    await enforceRepositoryGenerationRateLimit({
      repoFullName: ref.fullName,
      request,
    });
    generationRateLimitReserved = true;
  };
  ...
  if (repository === null) {
    ...
    try {
      await enforceGenerationRateLimitOnce();
  ...
  try {
    const start = await startRepositoryIndexing({
      beforeCreateJob: enforceGenerationRateLimitOnce,
      repository,
      request,
    });
